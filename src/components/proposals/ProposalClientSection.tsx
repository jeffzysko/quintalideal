...
import { formatCPF, formatCNPJ } from '@/lib/document-utils';
...
export function ProposalClientSection({ form, updateForm, errors = {} }: Props) {
  const isPJ = form.person_type === 'pj';
  const linkedToLead = !!form.lead_id;

  const handlePhoneChange = (value: string) => {
    updateForm({ client_phone: formatPhoneBR(value) });
  };

  const handleDocumentChange = (value: string) => {
    updateForm({ client_document: isPJ ? formatCNPJ(value) : formatCPF(value) });
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Input
              ...
              className={cn('mt-1.5', errors.client_document && 'border-destructive')}
              inputMode="numeric"
            />
            {errors.client_document && <p className="text-xs text-destructive mt-1">{errors.client_document}</p>}
          </div>
